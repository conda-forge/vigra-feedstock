...
template<class T> inline T * get_pointer(T * p)
{
    return p;
}
...
template<class T> inline T * get_pointer(std::auto_ptr<T> const& p)
{
    return p.get();
}
...
template<class T> inline T * get_pointer( std::unique_ptr<T> const& p )
{
    return p.get();
}
